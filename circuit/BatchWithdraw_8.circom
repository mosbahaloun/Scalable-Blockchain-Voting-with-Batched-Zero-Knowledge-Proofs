...
template Withdraw() {
    ...
    signal input root[8];
    signal input nullifierHash[8];
    signal input recipient[8];

    signal input secret[8][256];
    signal input nullifier[8][256];
    signal input hashPairings[8][10];
    signal input hashDirections[8][10];

    // Public outputs
    signal output pub_root[8];
    signal output pub_nullifierHash[8];
    signal output pub_recipient[8];

    component cHasher[8];
    component leafHashers[8][10];

    signal currentHash[8][11];
    signal left[8][10];
    signal right[8][10];
    signal recipientSquare[8];

...
describe("Full Test: 40 vote commitments (10×4), 10 vote submissions (4×), 10 recovery token batches (4×), with addCandidate", function () {
    let hasher, verifier, tornado, witnessCalc;
    let eaWallet;   // Simulated Election Authority signer

    // ---- constants ----
    const BATCH_SIZE     = 4;
    const TOTAL_VOTERS   = 40;                           // 10 × 4
    const NUM_COMMIT_TXS = TOTAL_VOTERS / BATCH_SIZE;   // 10 voteCommitment transactions
    const NUM_SUBMIT_TXS = TOTAL_VOTERS / BATCH_SIZE;   // 10 voteSubmission transactions

    const depositValue     = ethers.utils.parseEther("0.01");
    const batchCommitValue = depositValue.mul(BATCH_SIZE); // 0.04 ETH per voteCommitment
    ...
    const zkey         = path.join(__dirname, "../utils/setup_4_final.zkey");
    ...
    // expected by recoveryToken: [v(1)|r(32)|s(32)] × 4
    ...
    before(async () => {
        ...
        const Verifier = await ethers.getContractFactory("Groth4Verifier");
    ...
    // ================================================================
    // Test 1: voteCommitment — 10 txs of 4 notes each
    //         voteSubmission — 10 txs of 4 notes each
    // ================================================================
    it("should perform 10 voteCommitment txs (4 notes each) and 10 voteSubmission txs (BATCH=4)", async () => {
        ...
        // ---- 10 voteCommitment transactions (4 notes each = 40 voters total) ----
        for (let batchIdx = 0; batchIdx < NUM_COMMIT_TXS; batchIdx++) {
            ...
            const wStart = nowNs();
            // witness[1..4] = commitment[0..3]
            // witness[5..8] = nullifierHash[0..3]
            ...
            // Level-major [10][4] for Solidity uint256[4][10] / uint8[4][10]
            ...
            console.log(
                `✅ voteCommitment ${batchIdx + 1} (voters ${batchIdx * 4 + 1}–${batchIdx * 4 + 4}) Gas Used:`,
        ...
        // ---- 10 voteSubmission transactions (4 notes each) ----
        ...
        console.log("📊 Total Gas Used (10 voteCommitment of 4 + 10 voteSubmission of 4):", totalGas.toString());
        ...
        printStats("voteCommitment witness (BatchDeposit4 circuit, 4 notes)", commitWitnessMs);
        printStats("Merkle path compute (MiMC, 4 paths per batch)",           merklePathMs);
        printStats("voteCommitment inclusion (submit→mined, 10 txs)",         commitInclMs);
        printStats("voteSubmission proof gen (batch of 4)",                   submitProofMs);
    ...
    // ================================================================
    // Test 2: recoveryToken — 10 batches of 4 (40 voters total)
    //
    ...
    // ================================================================
    it("measures gas for 10 × recoveryToken calls (BATCH=4, 40 voters total)", async () => {
        const [owner] = await ethers.getSigners();
        const NUM_RECOVERY_BATCHES = 10; // 10 × 4 = 40 voters

        const recoveryGasReceipts = [];
        const recoveryInclMs      = [];

        for (let batch = 0; batch < NUM_RECOVERY_BATCHES; batch++) {
            const uniqueIds    = Array.from({ length: 4 }, () =>
                ethers.utils.hexZeroPad(ethers.utils.randomBytes(32), 32)
            );
            const recTokens    = Array.from({ length: 4 }, () =>
                ethers.utils.hexZeroPad(ethers.utils.randomBytes(32), 32)
            );
            const voterPubKeys = Array.from({ length: 4 }, () =>
                ethers.utils.hexZeroPad(ethers.utils.randomBytes(32), 32)
            );
            ...
            console.log(
                ...
                ` (voters ${batch * 4 + 1}–${batch * 4 + 4}) Gas Used:`,
                receipt.gasUsed.toString()
            );
        ...
        console.log("📊 Total gas (10 × recoveryToken of 4):", totalGas.toString());
        console.log("📈 Avg gas per call (4 tokens):",          totalGas.div(NUM_RECOVERY_BATCHES).toString());
        console.log("📈 Avg gas per token:",                    totalGas.div(NUM_RECOVERY_BATCHES * 4).toString());

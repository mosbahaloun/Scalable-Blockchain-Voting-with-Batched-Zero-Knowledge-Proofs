...
describe("Full Test: 40 vote commitments (5×8), 5 vote submissions (8×), 5 recovery token batches (8×), with addCandidate", function () {
    let hasher, verifier, tornado, witnessCalc;
    let eaWallet;   // Simulated Election Authority signer

    // ---- constants ----
    const BATCH_SIZE     = 8;
    const TOTAL_VOTERS   = 40;                           // 5 × 8
    const NUM_COMMIT_TXS = TOTAL_VOTERS / BATCH_SIZE;   // 5 voteCommitment transactions
    const NUM_SUBMIT_TXS = TOTAL_VOTERS / BATCH_SIZE;   // 5 voteSubmission transactions

    const depositValue     = ethers.utils.parseEther("0.01");
    const batchCommitValue = depositValue.mul(BATCH_SIZE); // 0.08 ETH per voteCommitment
    ...
    // expected by recoveryToken: [v(1)|r(32)|s(32)] × 8
    ...
    before(async () => {
        ...
        const Verifier = await ethers.getContractFactory("Groth8Verifier");
    ...
    // ================================================================
    // Test 1: voteCommitment — 5 txs of 8 notes each
    //         voteSubmission — 5 txs of 8 notes each
    // ================================================================
    it("should perform 5 voteCommitment txs (8 notes each) and 5 voteSubmission txs (BATCH=8)", async () => {
        ...
        // ---- 5 voteCommitment transactions (8 notes each = 40 voters total) ----
        for (let batchIdx = 0; batchIdx < NUM_COMMIT_TXS; batchIdx++) {
            ...
            // Level-major [10][8] for Solidity uint256[8][10] / uint8[8][10]
            ...
            console.log(
                `✅ voteCommitment ${batchIdx + 1} (voters ${batchIdx * 8 + 1}–${batchIdx * 8 + 8}) Gas Used:`,
        ...
        // ---- 5 voteSubmission transactions (8 notes each) ----
        ...
        console.log("📊 Total Gas Used (5 voteCommitment of 8 + 5 voteSubmission of 8):", totalGas.toString());
        ...
        printStats("voteCommitment witness (BatchDeposit8 circuit, 8 notes)", commitWitnessMs);
        printStats("Merkle path compute (MiMC, 8 paths per batch)",           merklePathMs);
        printStats("voteCommitment inclusion (submit→mined, 5 txs)",          commitInclMs);
        printStats("voteSubmission proof gen (batch of 8)",                   submitProofMs);
    ...
    // ================================================================
    // Test 2: recoveryToken — 5 batches of 8 (40 voters total)
    //
    ...
    // ================================================================
    it("measures gas for 5 × recoveryToken calls (BATCH=8, 40 voters total)", async () => {
        const [owner] = await ethers.getSigners();
        const NUM_RECOVERY_BATCHES = 5; // 5 × 8 = 40 voters

        const recoveryGasReceipts = [];
        const recoveryInclMs      = [];

        for (let batch = 0; batch < NUM_RECOVERY_BATCHES; batch++) {
            const uniqueIds    = Array.from({ length: 8 }, () =>
                ethers.utils.hexZeroPad(ethers.utils.randomBytes(32), 32)
            );
            const recTokens    = Array.from({ length: 8 }, () =>
                ethers.utils.hexZeroPad(ethers.utils.randomBytes(32), 32)
            );
            const voterPubKeys = Array.from({ length: 8 }, () =>
                ethers.utils.hexZeroPad(ethers.utils.randomBytes(32), 32)
            );
            ...
            console.log(
                ...
                ` (voters ${batch * 8 + 1}–${batch * 8 + 8}) Gas Used:`,
                receipt.gasUsed.toString()
            );
        ...
        console.log("📊 Total gas (5 × recoveryToken of 8):", totalGas.toString());
        console.log("📈 Avg gas per call (8 tokens):",         totalGas.div(NUM_RECOVERY_BATCHES).toString());
        console.log("📈 Avg gas per token:",                   totalGas.div(NUM_RECOVERY_BATCHES * 8).toString());

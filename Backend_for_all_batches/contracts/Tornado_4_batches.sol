...
interface IVerifier {
    function verifyProof(
        ...
        uint[2][2] memory b,
        uint[2] memory c,
        uint[12] memory input  // 4-batch withdraw: 4*3 = 12 public signals
    ) external view returns (bool);
}

contract Tornado_4 is ReentrancyGuard {
    address verifier;
    Hasher hasher;
    ...
    // BATCH_SIZE = 4, matching the BatchDeposit4 circuit
    ...
    // Emitted once per voteCommitment call (4 notes)
    event VoteCommitment(
        uint256[4] roots,
        uint256[4][10] hashPairings,
        uint8[4][10] pairDirections
    );
    ...
    // ---------------------------------------------------------------
    // Voter commitment — 4 notes per tx (matches BatchDeposit4 circuit)
    // ---------------------------------------------------------------

    /**
     * @notice Register 4 voter commitments in one transaction.
     * @param _commitments   commitment[0..3] from the BatchDeposit4 circuit
     * @param _newRoots      one updated Merkle root per inserted leaf
     * @param hashPairings   level-major [10][4] sibling hashes
     * @param hashDirections level-major [10][4] direction bits (0=left, 1=right)
     */
    function voteCommitment(
        uint256[4] calldata _commitments,
        uint256[4] calldata _newRoots,
        uint256[4][10] calldata hashPairings,
        uint8[4][10] calldata hashDirections
    ) external payable nonReentrant {
        // Must send exactly denomination × 4 (0.04 ETH)
    ...
    // ---------------------------------------------------------------
    // Privacy-preserving recovery token storage (4 tokens per tx)
    //
    ...
    /**
     * @notice Store 4 recovery token hashes on-chain in one transaction.
     *
     * @param uniqueIds    4 unique voter identifiers (256-bit random values)
     * @param recTokens    4 recovery tokens
     * @param voterPubKeys 4 voter ElGamal public keys (y_voterpubkey)
     * @param sigs         Packed EA signatures: [v(1)|r(32)|s(32)] × 4 = 260 bytes
     *                     where each sig covers keccak256(uniqueId_i || voterPubKey_i)
     */
    function recoveryToken(
        bytes32[4] calldata uniqueIds,
        bytes32[4] calldata recTokens,
        bytes32[4] calldata voterPubKeys,
        bytes calldata sigs        // 65 × 4 = 260 bytes
    ) external nonReentrant {
        require(electionAuthority != address(0), "ea-not-set");
        require(sigs.length == 65 * 4, "bad-sigs-length");
    ...
    // ---------------------------------------------------------------
    // 4-note vote submission
    // ---------------------------------------------------------------
    function voteSubmission(
        uint[2] memory a,
        uint[2][2] memory b,
        uint[2] memory c,
        uint[12] memory input,          // 4 roots | 4 nullifierHashes | 4 recipients
        address payable[4] memory recipients
    ) external payable nonReentrant {
        require(
        ...
        for (uint i = 0; i < 4; i++) {
            ...
            uint256 nullifierHash = input[4 + i];
